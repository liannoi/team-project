...
namespace TeamProject.Clients.Common.Models.Storage.Actors.ViewModels
{
    public class ActorBindingModelValidator: AbstractValidator<ActorBindingModel>
    {
        public ActorBindingModelValidator()
        {
            RuleFor(e => e.FirstName)
                .NotNull().WithMessage("Параметр FirstName не должно быть пустым или null").MinimumLength(3).WithMessage("FirstName must be longer than 2");
